...
{
	partial class FlightInformationViewController1 : BaseViewController
	{

		private UINavigationController thisController { get; set; }

		public FlightInformationViewController1 (IntPtr handle) : base (handle)
		{
			Title = "LOCATION";
		}
		...
		public override void ViewDidLoad ()
		{
			...
			edtPickUpFlight.ShouldReturn += TextFieldShouldReturn;
		...
		private void SetBindings(){
			...
			SetupAirlines (edtPickUpAirlines, KvpAirlines, 1);

			#endregion 

			SetupTimePicker (edtRequestedArrivalTimeAndDate, UIDatePickerMode.DateAndTime, "{0: MM/dd/yyyy hh:mm tt}", futureDatesOnly: true);
			SetupTimePicker (edtPickUpFlightTime, UIDatePickerMode.Time, "{0: hh:mm tt}");

			SetupFlightType (edtPickUpFlightType, true);
			...
			this.SetBinding (
				() => edtPickUpFlight.Text,
				() => Facade.Instance.CurrentRide.PickUpFlightNumber,
				BindingMode.TwoWay)
				.UpdateSourceTrigger ("EditingChanged");

			this.SetBinding (
				() => edtPickUpFlightTime.Text,
				() => Facade.Instance.CurrentRide.PickUpFlightTime,
				BindingMode.TwoWay)
				.UpdateSourceTrigger ("ValueChanged");
		...
		private void SetBindingsOnce(){
			...
			this.SetBinding (
				...
				.WhenSourceChanges (
					() => {
						edtPickUpAirlines.Text = "";
						edtPickUpFlightType.Text = "";
					});
		}
