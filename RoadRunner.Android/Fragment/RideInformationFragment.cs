...
namespace RoadRunner.Android
{
	public class RideInformationFragment : Fragment
	{
		...
		{
			mView = inflater.Inflate(Resource.Layout.fragment_rideInformation, container, false);

			AppSettings.currentActivity = this.Activity as MainActivity;
			mActivity = this.Activity as MainActivity;

			//pickup location
			m_txtPickupLocation = (TextView)mView.FindViewById(Resource.Id.txtPickupLocation);
			...
			//dropoff location
			m_txtDropoffLocation = (TextView)mView.FindViewById(Resource.Id.txtDropoffLocation);
			...
			//passengers or hours
			m_switchPH = (Switch)mView.FindViewById(Resource.Id.switchPorH);
			...
			m_spinnerPassengers = (Spinner)mView.FindViewById(Resource.Id.spPassengers);
			m_spinnerHours = (Spinner)mView.FindViewById(Resource.Id.spHours);
			var adapter = new ArrayAdapter(mActivity, global::Android.Resource.Layout.SimpleSpinnerItem);
			...
			//upto of vehicle
			m_rgUpto = (RadioGroup)mView.FindViewById(Resource.Id.rgUpto);
			...
			//type of vehicle
			m_rgVehicle = (RadioGroup)mView.FindViewById(Resource.Id.rgVehicle);
			...
			m_txtComment = (TextView)mView.FindViewById(Resource.Id.txtComment);
			m_txtComment.Text = "Complete all fields above";

			var btnGotoLocation = (Button)mView.FindViewById(Resource.Id.btn_GoToLocation1);
		...
		private void SetRideType(int selectedIndex)
		{
			...
			var btnSwitch1 = (RadioButton)mView.FindViewById(Resource.Id.rb1);
			var btnSwitch2 = (RadioButton)mView.FindViewById(Resource.Id.rb2);
			var btnSwitch3 = (RadioButton)mView.FindViewById(Resource.Id.rb3);
		...
		private async void GetFares()
		{
			...
			mActivity.ShowLoadingView("Retrieving Available Vehicles...");
			...
			if (m_fares.Count == 0)
			{
				...
				m_txtComment.Text = "Complete all fields above";
				mActivity.ShowMessageBox("Fares Error", "No fares available for the selected pickup/dropoff. Please confirm they are correct and try again.");
		...
		private List<FareContainer> GetFareForReservationPoint()
		{
			...
			{
				mActivity.HideLoadingView();
				mActivity.ShowMessageBox("Error", "An error occurred getting fares. \n\nError: " + ex.Message);
				...
				.WhenSourceChanges(
					async () =>
					{
						if (Facade.Instance.CurrentRide.CanGoToTheFlightInformation)
						{
							...
							if (Facade.Instance.CurrentRide.IsPickUpLocationAirport)
							{
						AppSettings.currentActivity.StartActivity(new Intent(AppSettings.currentActivity, typeof(LocationPickupActivity)));
							...
							{
								AppSettings.currentActivity.StartActivity(new Intent(AppSettings.currentActivity, typeof(LocationDropoffActivity)));
								AppSettings.currentActivity.OverridePendingTransition(Resource.Animation.fromLeft, Resource.Animation.toRight);
							}
							else {
								AppSettings.currentActivity.StartActivity(new Intent(AppSettings.currentActivity, typeof(LocationZipActivity)));
								AppSettings.currentActivity.OverridePendingTransition(Resource.Animation.fromLeft, Resource.Animation.toRight);
							}
						}
						else {
